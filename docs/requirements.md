...
- AC4: Saved jobs and applied jobs are listed on **`/applications`**, grouped by the stage each one is in. Reworded 2026-09-22: this said "on the profile page", and was satisfied by a `/saved-jobs` page reached from the account menu holding two flat lists. US-7.1 added the funnel, which lists the same rows in a strictly more informative shape — "where is each one" contains "what have I saved or applied to". Keeping both left two views of one list that could disagree, so the older one was removed and its bookmark and discard controls moved onto the funnel's rows. The intent is unchanged: every saved and applied job is findable in one place away from the job browser.
...
- AC1: Questions are generated from the target role and my actual profile, not from a fixed bank. *Note 2026-09-23 (9.2): taken to mean all three of the role's real demand, the specific posting, and the candidate's resume. Topics come from aggregating `job_skills` over live postings matching the role — the same aggregation skill gaps uses — falling back to a generic list below three matching postings and saying which it gave. The model must name the exact resume words it builds on, verified present before the question is stored; where it cannot, the question falls back to topic-only and the row records `degraded = true`. There is no fallback bank: one that appears only when the model fails is still a bank. Note 2026-09-23 (9.5, from a run against the real model): there are **three** grounding states, not two, and the third was invisible. A question can be grounded, or `degraded` because a claimed grounding failed verification, or simply ungrounded — the model was given the resume and volunteered no connection. Asked to build an AI Engineer question on a backend engineer's payments resume, Gemini returned `grounded_in: null`, which is ADR-012 holding rather than failing. But a page claiming questions are built from your resume cannot show that one in silence, because silence reads as the grounded case with the quote left off. The UI now says which of the three it is. Note 2026-09-24 (9.6): **this AC was only half met, and the half that was missing was the important one.** `blueprint_for` took a role *string*, so targeting a specific job added its raw text to the prompt as background reading and changed not one topic -- an interview "for this job" examined the corpus average. Topics now come from the chosen posting's own `job_skills`, weighted REQUIRED over PREFERRED, falling back to role demand only when it yields fewer than `MIN_JOB_TOPICS`. Three entry points reach it: paste a posting (link optional), pick a job you applied to, or name a role and get aggregate demand. And the AC's own promise of "saying which it gave" is now kept at the API: `InterviewTopicSource` on the row and in `InterviewRead`, because `blueprint.py` had claimed in prose since 9.2 that it reached the API and nothing carried it past the function that computed it. Note 2026-09-24 (9.7): the topics now also lean towards what the candidate *cannot* do, reusing the skill-gap service's own held/partial/missing answer -- a preference rather than a filter, so the job's own emphasis stays primary. AC1 asks for questions from the role **and my actual profile**, and until this the profile shaped only how a question was phrased, never which topic it was about.*
- AC2: The session persists and can be resumed. *Met by ADR-013's decision to hold the state machine in Postgres rather than in a model's context window: `GET /interviews/{id}` returns the same transcript, difficulty and covered topics tomorrow. Note 2026-09-23 (9.5): that was only half of it. The state survived and there was no route back to it, so "resumable" was true only for somebody who had kept the URL — `GET /interviews` and the Practice page are the way back, and the claim is now true for a person rather than for a client.*
...
- AC2: Feedback cites the specific part of my answer it refers to. *Character offsets into the answer, verified to lie inside it before storage, with the stored text taken from the answer rather than from the model's account of it. A citation pointing elsewhere than it claims is worse than none, because it will be believed. Note 2026-09-23 (9.5): shown as highlights inside the answer on the transcript, with the marked text rendered from the answer at those offsets rather than from `span.text` — so a disagreement between the two could not reach a reader even if the server check ever stopped. Overlapping spans drop rather than duplicating the candidate's own words.*
- AC3: Scoring is validated against a human-labelled test set with reported agreement. *Harness built 2026-09-23 (9.4): 100 answers to 20 questions in `ml/datasets/interview_scoring/`, written so the five dimensions disagree with each other, plus constant and length baselines and the Pearson/MAE/Spearman report. **Blocked on the marking**, which is a person's job by definition — a model scored against its own marks agrees with itself. `run_scoring_eval` prints what it needs and produces no number until then.*
