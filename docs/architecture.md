...
| Skill extraction | **Discharged 2026-09-12** — 30 hand-labelled job postings, 511 gold skills | P 0.767 / R 0.909 / **F1 0.832** after filling the taxonomy gaps and requiring generic terms to appear where skills are listed; recall and F1 targets met, precision not |
...
| 2026-09-10 | The separate `/recommendations` page is withdrawn one commit after shipping; match ranking becomes `GET /jobs?sort=match` and a "Sort by" control in the Jobs filter panel. Splitting them had left the ranking with none of the seven browse filters and browse with no ranking, so "remote Python jobs, best match first" was inexpressible — the split was the defect, not the missing page. Browse filters are now one definition (`active_job_conditions`) shared by the SQL browse query and the vector recall, pushed into stage one so a filter narrows the set *before* the 200-row recall cap rather than after. Offset paging in both modes, because the browse list's page lives in its URL and a cursor cannot be written as `?offset=`. `/recommendations` survives for the dashboard panel and the feedback endpoint. |
| 2026-09-12 | ADR-015's skill-extraction row discharged, and the result reframes an earlier one. Recall 0.948 beats its 0.80 target; precision 0.670 misses 0.85 — but ~4 in 5 counted false positives are terms literally present in the posting, so the figure measures a disagreement about what counts as a skill rather than hallucination. The extractor cannot separate "the job involves security" from "security is required", which is the cause of the `Communication`-in-45%-of-postings problem that 6.5 treated symptomatically with rarity weighting. Separately, **24% of gold skills have no taxonomy entry at all** (NoSQL, DevOps, MLOps, Databricks, Django REST Framework) — a hard recall ceiling, and the cheapest available improvement to the whole system. The naive baseline is beaten on recall and F1 but *not* on precision. The dataset is 30 job postings rather than ml.md's 50 resumes: three distinct resume texts exist, the matcher is shared between both paths, and the resume-side section weighting stays unmeasured. |
