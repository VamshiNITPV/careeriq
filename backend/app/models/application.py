...
class Application(Base, UUIDPrimaryKeyMixin, TimestampMixin, SoftDeleteMixin):
    ...
    **Two independent facts, not one.** `is_saved` is the user's bookmark;
    ...
    __table_args__ = (
        ...
        # Declared here as well as in migration 0020, which is the point:
        # `compare_server_default=True` makes autogenerate compare the model to
        # the database, so an index that exists only in a migration reads as
        # drift and the next autogenerated revision proposes dropping it. Found
        # exactly that way, one migration later.
        Index("ix_applications_resume_version_id", "resume_version_id"),
